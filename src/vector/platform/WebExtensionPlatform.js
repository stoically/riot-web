...
class SeshatIndexManager extends BaseEventIndexManager {
    ...
    async searchEventIndex(config: SearchConfig): Promise<SearchResult> {
    ...
    async removeCrawlerCheckpoint(oldCheckpoint: CrawlerCheckpoint): Promise<> {
        return browser.runtime.sendMessage({
            type: "seshat",
    ...
    async deleteEvent(eventId): Promise<> {
        return browser.runtime.sendMessage({
            type: "seshat",
            method: "deleteEvent",
            content: { eventId },
...
export default class WebExtensionPlatform extends WebPlatform {
    ...
    async startSingleSignOn(mxClient: MatrixClient, loginType: "sso" | "cas") {
        ...
        const loginUrl = this.getSSOCallbackUrl(
            homeserverUrl,
